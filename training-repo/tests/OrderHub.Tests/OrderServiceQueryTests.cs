...
public class OrderServiceQueryTests
{
    ...
    /// <summary>
    /// Regression: page is 1-based. Page 1 must include the newest order;
    /// last page must not be empty when totalCount is an exact multiple of pageSize.
    /// </summary>
    [Fact]
    public async Task GetOrders_Page1_ReturnsNewestOrders_NotSkipped()
    {
        using var db = TestSetup.CreateContext();
        var service = TestSetup.CreateOrderService(db);
        var customer = TestSetup.AddCustomer(db);
        ...
        Assert.DoesNotContain(page1.Items.Select(o => o.Id), id => page2.Items.Any(o => o.Id == id));
    }
